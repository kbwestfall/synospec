...
class FOBOSETC(scriptbase.ScriptBase):

    @classmethod
    def get_parser(cls, width=None):
        import argparse

        parser = super().get_parser(description='FOBOS Exposure-Time Calculator (v0.2)',
                                    width=width)
        ...
        parser.add_argument('--ignore_mag', default=False, action='store_true',
                            help='Ignore the provided magnitude of the source and use the '
    ...
    def main(args):
        ...
        spec = get_spectrum(wave, args.mag, mag_band=args.mag_band, mag_system=args.mag_system,
                            ignore_mag=ignore_mag, spec_file=args.spec_file,
                            spec_wave=args.spec_wave, spec_wave_units=args.spec_wave_units,
                            spec_flux=args.spec_flux, spec_flux_units=args.spec_flux_units,
                            spec_res_indx=args.spec_res_indx, spec_res_value=args.spec_res_value,
                            spec_table=args.spec_table, emline_db=emline_db,
                            redshift=args.redshift, resolution=resolution)
    ...
    #    ax = sky_spectrum.plot(ax=ax, show=True)
        ...
        print('Sky g- and r-band AB surface brightness: {0:.3f} {1:.3f} {2:.3f}'.format(
                        ...
                        sky_spectrum.magnitude(band=iband)))
