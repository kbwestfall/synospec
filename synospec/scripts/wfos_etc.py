...
class WFOSETC(scriptbase.ScriptBase):

    @classmethod
    def get_parser(cls, width=None):
        import argparse

        parser = super().get_parser(description='WFOS Exposure-Time Calculator (v0.1)',
                                    width=width)
        ...
        dist_group.add_argument('-s', '--sersic', default=None, nargs=4, type=float,
                                help='Use a Sersic profile to describe the object '
                                     'surface-brightness distribution; order must be effective '
                                     'radius, Sersic index, ellipticity (1-b/a), position angle '
                                     '(deg).')
        dist_group.add_argument('-u', '--uniform', default=False, action='store_true',
                                help='Instead of a point source or Sersic profile, assume the '
                                     'surface brightness distribution is uniform over the fiber '
                                     'face.  If set, the provided magnitude is assumed to be a '
                                     'surface brightness.  See the MAG option.')

        parser.add_argument('--refl', default='req', type=str,
                            help='Select the reflectivity curve for TMT.  Must be either \'req\' '
                                 'or \'goal\' for the required or goal reflectivity performance.')
        ...
        bgrat_grp.add_argument('--blue_angle', default=None, type=float,
                               help='Grating angle for blue grating.  If None, will use then '
        ...
        parser.add_argument('--blue_binning', default=[1,1], nargs=2, type=int,
                            help='On-chip binning for the blue grating.  Order is spectral then '
                                 'spatial.  I.e., to bin 2 pixels spectrally and no binning '
                                 'spatial, set --blue_binning 2 1')

        parser.add_argument('--red_grat', default=TMTWFOSRed.default_grating, type=str,
                            help='Grating to use in the red arm.  Options are: {0}'.format(
                                ', '.join([g for g in WFOSGrating.available_gratings.keys()
                                            if 'R' in g])))
        ...
        parser.add_argument('--slit', default=[0., 0., 0.75, 5.0, 0.0], nargs=5, type=float,
                            help='Slit properties: x field center, y field center, width, length, '
                                 ...
                                 'dispersion direction with a valid range of +/- 90 arcsec, and y '
                                 'is in the cross-dispersion direction with a valid range of +/- '
                                 '249 arcsec.  Coordinate (0,0) is on axis.')
        parser.add_argument('--extract', default=None, type=float,
                            help='Extraction aperture in arcsec *along the slit* centered on the '
                                 'source.  At the detector, the extraction aperture is narrower '
                                 'by cos(slit rotation).  If not provided, set to the FWHM of the '
    ...
    def main(args):
        ...
        #-------------------------------------------------------------------
        # Blue Arm
        blue_arm = TMTWFOSBlue(reflectivity=args.refl, grating=args.blue_grat,
        ...
        spatial_width = slit.length * numpy.cos(numpy.radians(slit.rotation)) \
                            / blue_arm.pixelscale / args.blue_binning[1]
        ...
        # Setup dummy wavelength vectors to get something appropriate for sampling
        max_resolution = red_arm.resolution(red_wave_lim[1], x=slit_x,
                                            slit_width=effective_slit_width)
        ...
        red_wave = get_wavelength_vector(red_wave_lim[0], red_wave_lim[1],
                                         numpy.mean(red_ang_per_pix), linear=True)
        ...
        # Extraction width in pixels
        spatial_width = slit.length * numpy.cos(numpy.radians(slit.rotation)) \
                            / red_arm.pixelscale / args.red_binning[1]
