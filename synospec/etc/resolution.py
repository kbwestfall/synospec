...
class SpectralResolution:
    ...
    Attributes:
        ...
        sig_mask (`numpy.ndarray`):
            A boolean vector used to identify measurements of
...
#        self.sig_mask = numpy.array(self.sig_pd < self.min_sig).astype(numpy.uint)
        self.sig_mask = self.sig_pd < self.min_sig
#        print('finalize e')
...
#        return self.interpolator._data[1]
    ...
    def instrumental_dispersion(self, w=None):
        ...
        if w is None:
            return numpy.ma.divide(self.c, sig2fwhm*self.interpolator.y).filled(0.0)
        return numpy.ma.divide(self.c, sig2fwhm*self.interpolator(w)).filled(0.0)
...
#        print('sig2_wd')
        sig2fwhm = numpy.sqrt(8.0 * numpy.log(2.0))
        sig2_wd = numpy.square(_wave/sig2fwhm) \
...
#            self.ZeroGaussianKernelDifference()
    ...
    def adjusted_resolution(self, indx=None):
        ...
        sig2fwhm = numpy.sqrt(8.0 * numpy.log(2.0))
        if indx is None:
            return 1.0/numpy.sqrt( numpy.square(sig2fwhm/self.c) \
...
def match_spectral_resolution(wave, flux, sres, new_sres_wave, new_sres, ivar=None, mask=None, 
    ...
    Args:
        ...
        mask (numpy.ndarray): (**Optional**) A 1D or 2D (:math:`N_{\rm
            spec}\times N_{\rm pix}`) array with a boolean mask for the
    ...
    Returns: 
        numpy.ndarray: Five objects are returned:
            ...
            - A 1D vector with any constant offset in resolution **in
              ...
              :func:`SpectralResolution.GaussianKernelDifference`.
            - A 1D or 2D (:math:`N_{\rm spec}\times N_{\rm pix}`) array
              with a boolean mask for the resolution-matched flux sampled
...
#            pyplot.show()
    ...
    out_sres = sres.copy()
    _mask = numpy.zeros(flux.shape, dtype=bool) if mask is None else mask.astype(bool)
    out_mask = _mask.copy()
...
#    print('test div by zero')
    if nspec == 1 and spec_dim == 1:
        ...
        try:
            ...
            out_mask = numpy.ones(flux.shape, dtype=bool)
    else:
...
#                indx = numpy.where(res[i].sig_pd > min_sig_pix)
                ...
                if ivar is None:
                    ...
                    del oflux, oivar
                out_mask[i,:] = (res[i].sig_mask == 1) | _mask[i,:]
                ...
                warnings.warn('Encountered ValueError: {0} ; continuing but resolution is NOT '
                              'changed and mask is set.'.format(e))
                out_mask[i,:] = numpy.ones(flux.shape[1], dtype=bool)
        if not quiet:
